...
public class AudienceValidator implements OAuth2TokenValidator<Jwt> {
  ...
  public AudienceValidator(Collection<String> requiredAudiences) {
    this.requiredAudiences = requiredAudiences == null ? List.of() : List.copyOf(requiredAudiences);
  }
  ...
  public OAuth2TokenValidatorResult validate(Jwt token) {
    ...
    boolean match = audiences != null && audiences.stream().anyMatch(requiredAudiences::contains);
    return match ? OAuth2TokenValidatorResult.success() : OAuth2TokenValidatorResult.failure(ERROR);
  }
}
