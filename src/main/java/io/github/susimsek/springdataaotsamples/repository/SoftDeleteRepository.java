...
public interface SoftDeleteRepository<T extends SoftDeletableEntity, ID> {
    ...
    @Transactional
    @Query(
            "update #{#entityName} e set e.deleted = true, e.deletedBy = :#{authentication?.name},"
                + " e.deletedDate = :#{T(java.time.Instant).now()} where e.id in :ids and e.deleted"
                + " = false")
    ...
    @Query(
            "update #{#entityName} e set e.deleted = true, e.deletedBy = :#{authentication?.name},"
                + " e.deletedDate = :#{T(java.time.Instant).now()} where e.id = :id and e.deleted ="
                + " false")
    ...
    @Transactional
    @Query(
            "update #{#entityName} e set e.deleted = false, e.deletedBy = null, e.deletedDate ="
                    + " null where e.id in :ids and e.deleted = true")
    ...
    @Query(
            "update #{#entityName} e set e.deleted = false, e.deletedBy = null, e.deletedDate ="
                    + " null where e.id = :id and e.deleted = true")
