...
public interface SoftDeleteRepository<T extends SoftDeletableEntity, ID> {
    ...
    @Modifying
    @Transactional
    @Query("update #{#entityName} e set e.deleted = true, e.deletedBy = ?#{@auditorAware.currentAuditor.orElse('system')}, e.deletedDate = ?#{T(java.time.Instant).now()} where e.id in :ids and e.deleted = false")
    int softDeleteByIds(@Param("ids") Iterable<ID> ids);

    @Modifying
    @Transactional
    @Query("update #{#entityName} e set e.deleted = true, e.deletedBy = ?#{@auditorAware.currentAuditor.orElse('system')}, e.deletedDate = ?#{T(java.time.Instant).now()} where e.id = :id and e.deleted = false")
