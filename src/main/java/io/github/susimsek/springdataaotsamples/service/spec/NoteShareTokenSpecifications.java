...
public class NoteShareTokenSpecifications {
    ...
    public static Specification<NoteShareToken> status(String status) {
        ...
        return (root, cq, cb) -> switch (val) {
            ...
            case "expired" -> cb.and(
                    ...
                    cb.lessThan(root.get(NoteShareToken_.expiresAt), java.time.Instant.now())
            );
            case "active" -> cb.and(
                    cb.isFalse(root.get(NoteShareToken_.revoked)),
                    cb.or(
                            cb.isNull(root.get(NoteShareToken_.expiresAt)),
                            cb.greaterThan(root.get(NoteShareToken_.expiresAt), java.time.Instant.now())
                    )
            );
    ...
    public static Specification<NoteShareToken> createdBetween(@Nullable Instant from,
                                                               @Nullable Instant to) {
        ...
        return (root, cq, cb) -> {
            ...
            if (from != null) {
                predicate = cb.and(predicate, cb.greaterThanOrEqualTo(root.get(AuditableEntity_.createdDate), from));
            }
            if (to != null) {
                predicate = cb.and(predicate, cb.lessThanOrEqualTo(root.get(AuditableEntity_.createdDate), to));
            }
            return predicate;
        };
    }
