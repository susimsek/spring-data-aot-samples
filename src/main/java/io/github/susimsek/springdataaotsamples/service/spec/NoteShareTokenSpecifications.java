          ...
          case "active" ->
              cb.and(
                  ...
                  cb.or(
                      ...
                      cb.greaterThan(
                          root.get(NoteShareToken_.expiresAt), java.time.Instant.now())));
  ...
  public static Specification<NoteShareToken> createdBetween(
    ...
    return (root, cq, cb) -> {
      ...
      if (from != null) {
        predicate =
            cb.and(
                predicate, cb.greaterThanOrEqualTo(root.get(AuditableEntity_.createdDate), from));
      }
      if (to != null) {
        predicate =
            cb.and(predicate, cb.lessThanOrEqualTo(root.get(AuditableEntity_.createdDate), to));
      }
      return predicate;
    };
  }
